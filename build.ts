...
// ---------------------------------------------------------------------------
// Fix Bun Duplicate Export Bug
//   Bun has a bug (issues #5344, #10631) where it adds duplicate exports.
//   This function removes the duplicate single-symbol exports.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Inline Worker: Create a module that exports the worker code as a string
//   This allows the library to work with any bundler without configuration.
//   The worker is created from a Blob URL at runtime instead of loading a file.
//   Always uses the minified version for smaller bundle size.
//   Returns the inline module content to be used during build.
// ---------------------------------------------------------------------------
function createInlineWorker(): string {
  const workerMinPath = path.join(DIST, "gpu-worker.min.js");
  const workerPath = path.join(DIST, "gpu-worker.js");

  // Prefer minified version, fallback to regular if needed
  const workerFile = fs.existsSync(workerMinPath) ? workerMinPath : workerPath;

  if (!fs.existsSync(workerFile)) {
    throw new Error(
      "Worker file not found, cannot inline. Make sure worker is built first.",
    );
  }

  // Read minified worker code
...
// This file contains the inlined MINIFIED worker code for maximum bundler compatibility
export const WORKER_CODE = ${JSON.stringify(workerCode)};
`;

  const isMinified = workerFile === workerMinPath;
  console.log(
    `✅ Worker inlined (${isMinified ? "minified" : "regular"}, ${(workerCode.length / 1024).toFixed(1)} KB)`,
  );
...
async function buildLib() {
  ...
  const entries = [
    ...
    path.join(SRC, "gpu-worker.ts"), // Needed for inlining, will be removed later
  ...
  // Minified build → .min.js (splitting enables code reuse and smaller bundles)
  ...
  const inlineWorkerCode = createInlineWorker();
  if (!inlineWorkerCode) {
